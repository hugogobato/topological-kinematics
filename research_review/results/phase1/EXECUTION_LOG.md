...
## Phase 2 and Phase 3 outcomes (2026-09-21)

| Date/time | Gate | Decision | Evidence |
|---|---|---|---|
| 2026-09-21 01:00 | G1 | CONDITIONAL GO for Phase 2 | research_review/results/phase1/G1_decision.md, verification/G1_audit.md |
| 2026-09-21 02:00 | G2 | PASS with residuals, metric backend amended | research_review/results/g2/G2_decision.md, diagnostics/metric_backend_audit.md |
| 2026-09-21 04:20 | G3 (exploratory H0) | PIVOT, confirmatory not authorized | research_review/results/g3/gate_memo.md, claims_matrix.yaml, g3_decision.json, verification/G3_audit.md |

### Metric amendment

The frozen gudhi 3.12.0 bottleneck backend is order-dependent and wrong on some real inputs
(witness: exact 0.3985347143760231 against canonical-order 0.6650287460769155). The primary metric
is now `bottleneck_exact`, an augmented-matching binary search validated against the project brute
force, persim, and an order-invariance suite (`tests/test_metric_exact.py`). Affected caches and
artifacts were regenerated; `bottleneck_gudhi` is retained for audit only.

### Cache concurrency repair

Trajectory and diagram caches now use per-cache file locks with atomic file-level commits and
corruption-tolerant loads. An 8-process stress test with a pre-seeded corrupt cache passes
(`/tmp/opencode/stress_cache.py`), and the full suite passes.

### Exploratory result and G3

The exploratory grid (20/10/20 base seeds, sigma in {0, 0.05}, strides 1, 2, 4, degree H0 both
families) completed in 839.7 s wall at 8 workers with 292 MB peak RAM. Every one of the twelve
cells has zero macro balanced error for both the compact representation and the validation
incumbent `raw_geometry_flat`; delta [0, 0]; parsimony ratio 0.615 below the fourfold threshold.
The predeclared rule returns PIVOT and the confirmatory stage was not opened; seeds 10000 and
above remain untouched. The H1 secondary channel pass reached 176 of about 300 distance matrices
and is recorded as not completed. WP-3.3 nulls and stress controls completed with the
equal-speed ordering separation result.

### Environment note

The WSL environment kills long-lived background process trees intermittently. All Phase 3 runs
were executed in supervised foreground chunks with cache-based resumption; wall-clock figures in
the artifacts are the measured stage times, not the supervised campaign time.
